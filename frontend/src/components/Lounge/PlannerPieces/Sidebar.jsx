...
export default function Sidebar({
  ...
  const menuSections = [
    {
      title: "MAIN",
      items: [
        { id: "overview", label: "Overview", icon: <Sparkles size={20} />, badge: null },
        { id: "events", label: "Event Board", icon: <CalendarDays size={20} />, badge: null },
      ]
    },
    {
      title: "COMMUNICATIONS",
      items: [
        { id: "requests", label: "Pending Requests", icon: <Inbox size={20} />, badge: counts.requests || null },
        { id: "messages", label: "Messages", icon: <MessageSquare size={20} />, badge: counts.messages || null },
        { id: "notifications", label: "Notifications", icon: <Bell size={20} />, badge: counts.notifications || null },
      ]
    },
    {
      title: "ACCOUNT",
      items: [
        { id: "profile", label: "My Profile", icon: <User size={20} />, badge: null },
      ]
    },
  ];
  ...
  return (
    <>
      {/* Mobile Overlay */}
      {isOpen && <div className="md:hidden fixed inset-0 bg-black bg-opacity-50 z-30" onClick={() => setIsOpen(false)} />}
      ...
      <aside
        ...
        } ${isCollapsed ? "w-20" : "w-64"} shadow-xl border-r border-brand-gold/20`}
      >
        {/* Header */}
          ...
          <div className={`flex items-center justify-between ${isCollapsed ? 'flex-col space-y-2' : 'flex-row'}`}>
            <div className={`flex items-center ${isCollapsed ? 'flex-col space-y-2' : 'space-x-3'}`}>
              <div className="p-2 bg-brand-gold rounded-lg">
                <Building2 className="text-brand-navy" size={isCollapsed ? 20 : 24} />
              </div>
              {!isCollapsed && (
                <div>
                  <h1 className="text-xl font-bold text-brand-gold">{companyName}</h1>
          ...
          <div className="p-4 border-b border-brand-gold/20">
            <div className="flex items-center space-x-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-brand-ivory/60 truncate">Event Planner</p>
              </div>
            </div>
          </div>
        )}
            ...
            <div key={section.title}>
              {!isCollapsed && (
                <div className="px-4 mb-2">
                  <p className="text-xs font-semibold text-brand-gold/60 uppercase tracking-wider">{section.title}</p>
                </div>
              )}
